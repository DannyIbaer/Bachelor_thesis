...
```
```{r}
#Function to create pathway lists for UpSet plots
 create_pathway_lists <- function(pathways_df) {
 if(nrow(pathways_df) == 0) return(list())

 # Improved cleaning: Remove ALL numeric suffixes after dots
 pathways_df$Condition <- sub("\\.[0-9]+$", "", pathways_df$Cluster)

 print("Cleaned conditions:")
 print(unique(pathways_df$Condition))

 pathway_list <- split(pathways_df$Pathway_DB, pathways_df$Condition)
 return(pathway_list)
}
```

```{r}
# Create pathway lists for each database
go_pathway_list <- create_pathway_lists(go_pathways_df)
kegg_pathway_list <- create_pathway_lists(kegg_pathways_df)
reactome_pathway_list <- create_pathway_lists(reactome_pathways_df)
hallmarks_pathway_list <- create_pathway_lists(hallmarks_pathways_df)
dorothea_pathway_list <- create_pathway_lists(dorothea_pathways_df)
```
...
```{r}
# Enhanced function to extract overlapping pathway details with descriptions
extract_overlapping_pathway_details <- function(all_pathways_df, all_pathway_lists) {
  library(openxlsx)
  
  # Get the binary matrix for intersections
  binary_matrix <- fromList(all_pathway_lists)
  conditions <- colnames(binary_matrix)
  
  # Initialize list to store detailed overlaps
  detailed_overlaps <- list()
  
  # Extract pairwise overlapping pathways with full details
  for (i in 1:(length(conditions)-1)) {
    for (j in (i+1):length(conditions)) {
      cond1 <- conditions[i]
      cond2 <- conditions[j]
      
      # Find pathways that appear in both conditions
      common_pathways <- intersect(all_pathway_lists[[cond1]], all_pathway_lists[[cond2]])
      
      if (length(common_pathways) > 0) {
        # Extract detailed information for each overlapping pathway
        overlap_details <- data.frame()
        
        for (pathway in common_pathways) {
          # Find this pathway in the original combined dataframe
          pathway_info <- all_pathways_df[all_pathways_df$Pathway_DB == pathway, ]
          
          if (nrow(pathway_info) > 0) {
            # Get the first occurrence for basic info
            pathway_row <- pathway_info[1, ]
            
            overlap_details <- rbind(overlap_details, data.frame(
              Condition1 = cond1,
              Condition2 = cond2,
              Pathway_Name = pathway,
              Database = pathway_row$Database,
              Description = pathway_row$Description,
              Gene_Count = pathway_row$Count,
              P_Adjust = pathway_row$p.adjust,
              Gene_Ratio = pathway_row$GeneRatio,
              BG_Ratio = pathway_row$BgRatio,
              Genes_Involved = pathway_row$geneID,
              stringsAsFactors = FALSE
            ))
          }
        }
        
        if (nrow(overlap_details) > 0) {
          comparison_name <- paste(cond1, "vs", cond2, sep = "_")
          detailed_overlaps[[comparison_name]] <- overlap_details
        }
      }
    }
  }
  
  return(detailed_overlaps)
}

# Extract detailed overlapping pathway information
detailed_pathway_overlaps <- extract_overlapping_pathway_details(all_pathways_df, all_pathway_lists)
...
summary_data <- data.frame(
  Comparison = names(detailed_pathway_overlaps),
  Number_of_Overlapping_Pathways = sapply(detailed_pathway_overlaps, nrow),
  stringsAsFactors = FALSE
)
writeData(wb, "Summary", summary_data)

# Add detailed sheets for each pairwise comparison
for (comparison_name in names(detailed_pathway_overlaps)) {
  # Clean sheet name for Excel compatibility
  sheet_name <- gsub("[^A-Za-z0-9_]", "_", comparison_name)
  sheet_name <- substr(sheet_name, 1, 31)  # Excel sheet name limit
  
  addWorksheet(wb, sheet_name)
  writeData(wb, sheet_name, detailed_pathway_overlaps[[comparison_name]])
}

# Save the workbook
saveWorkbook(wb, "detailed_overlapping_pathways.xlsx", overwrite = TRUE)
cat("Detailed overlapping pathways saved to: detailed_overlapping_pathways.xlsx\n")
